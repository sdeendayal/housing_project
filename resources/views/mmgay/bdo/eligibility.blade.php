...
@section('title', 'Awaiting Visit Schedule')
@section('page_header', 'Awaiting Schedule')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4 pb-3 border-b border-slate-100">
            <div>
                ...
                <p class="text-[10px] text-slate-400 mt-0.5 font-semibold uppercase">List of eligible MMGAY beneficiaries awaiting visit scheduling.</p>
            </div>
            <!-- Search bar - Denser -->
            <form action="{{ route('mmgay.bdo.eligibility-list') }}" method="GET" class="flex gap-2">
                ...
                @if($search)
                    <a href="{{ route('mmgay.bdo.eligibility-list') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center">Reset</a>
                @endif
            </form>
        </div>
