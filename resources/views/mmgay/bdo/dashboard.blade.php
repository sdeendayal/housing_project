...
<main class="ml-[260px] min-h-screen bg-[#f3f6fc] p-4 flex-1">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2.5 mb-4">
        ...
        <a href="{{ route('mmgay.bdo.possession-applications', ['status' => 'Site Verified']) }}" class="bg-white rounded-xl shadow-sm border border-slate-150 p-3 flex items-center justify-between hover:shadow hover:border-amber-200 transition font-semibold">
            <div class="min-w-0">
                ...
                @forelse($villages as $vil)
                    <div class="flex items-center justify-between p-3 rounded-lg border transition-all 
                       {{ $selectedVillageId == $vil->VillageId ? 'bg-blue-50 border-blue-200 text-blue-800 font-bold' : 'bg-slate-50 border-slate-150 text-slate-700 hover:bg-slate-100/70' }}">
                        <a href="{{ route('mmgay.bdo.dashboard') }}?phase={{ $selectedPhase }}&village_id={{ $vil->VillageId }}" class="flex items-center gap-2 flex-1 min-w-0">
