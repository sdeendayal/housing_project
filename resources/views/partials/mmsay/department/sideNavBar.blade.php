...
<style>
    @property --sidebar-orbit-angle {
        syntax: '<angle>';
        initial-value: 0deg;
        inherits: false;
    }

    /* Existing sidebar markup and active-link logic remain unchanged. */
    .sidebar-scroll a {
        position: relative;
        isolation: isolate;
    }

    .sidebar-scroll a::before {
        content: '';
        position: absolute;
        inset: 0;
        z-index: 20;
        ...
        pointer-events: none;
        opacity: 0;
        background: conic-gradient(from var(--sidebar-orbit-angle),
                #7c3aed,
                #2563eb,
                #06b6d4,
                #10b981,
    ...
    /* Rotate only on hover, keyboard focus or the existing active state. */
    .sidebar-scroll a:hover::before,
    .sidebar-scroll a:focus-visible::before,
    .sidebar-scroll a.bg-indigo-50::before {
        opacity: 1;
        animation: sidebar-border-orbit 1.6s linear infinite;
    }

    @keyframes sidebar-border-orbit {
        to {
            --sidebar-orbit-angle: 360deg;
        }
    }
    ...
    #departmentSidebar.is-mobile-open {
        transform: translateX(0);
    }

    #departmentSidebarOverlay {
        opacity: 0;
        visibility: hidden;
        transition: opacity 220ms ease, visibility 220ms ease;
    }

    #departmentSidebarOverlay.is-mobile-open {
        opacity: 1;
        visibility: visible;
    }
    ...
    @media (min-width: 1024px) {
        ...
        #departmentSidebarOverlay,
        #departmentSidebarToggle,
        #departmentSidebarClose {
...
<button type="button" id="departmentSidebarToggle" aria-label="Open navigation menu"
 aria-controls="departmentSidebar" aria-expanded="false"
 class="fixed left-3 top-3 z-[60] inline-flex h-11 w-11 items-center justify-center rounded-xl border border-indigo-100 bg-white text-indigo-600 shadow-lg shadow-slate-900/10 transition hover:bg-indigo-50 focus:outline-none focus:ring-4 focus:ring-indigo-100 lg:hidden">
<span class="material-symbols-outlined text-[22px]">menu</span>
</button>

<div id="departmentSidebarOverlay" class="fixed inset-0 z-40 bg-slate-950/45 backdrop-blur-[2px] lg:hidden"
    aria-hidden="true"></div>

<aside id="departmentSidebar" aria-label="Department navigation"

class="fixed inset-y-0 left-0 z-50 flex flex-col border-r border-slate-200 bg-white shadow-[4px_0_20px_rgba(15,23,42,0.12)]">
...
<div class="flex h-20 shrink-0 items-center gap-3 border-b border-slate-100 px-4">
    ...
    <button type="button" id="departmentSidebarClose" aria-label="Close navigation menu"
        class="ml-auto inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-slate-400 transition hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus:ring-4 focus:ring-slate-100 lg:hidden">
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const sidebar = document.getElementById('departmentSidebar');
        const overlay = document.getElementById('departmentSidebarOverlay');
        const openButton = document.getElementById('departmentSidebarToggle');
        ...
        const openSidebar = () => {
            ...
            overlay.classList.add('is-mobile-open');
        ...
        const closeSidebar = (restoreFocus = true) => {
            ...
            overlay.classList.remove('is-mobile-open');
            openButton.setAttribute('aria-expanded', 'false');
            overlay.setAttribute('aria-hidden', 'true');
            document.body.style.overflow = '';

            if (restoreFocus && window.innerWidth < 1024) openButton.focus();
        };
