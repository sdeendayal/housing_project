...
<form method="POST" action="{{ route('pp.department.login.verify') }}" id="ppDepartmentVerifyOtpForm" data-pp-loading>
    ...
    <div class="field">
        ...
        <input type="text" name="otp" id="ppDepartmentOtpInput" class="form-control text-center pp-otp-input @error('otp') is-invalid @enderror"
...
<div class="d-flex justify-content-between align-items-center small mt-2">
    <form method="POST" action="{{ route('pp.department.login.resend-otp') }}" class="m-0">
        @csrf
        <button type="submit" class="btn btn-link btn-sm p-0 text-decoration-none">Resend OTP</button>
    </form>
    <a href="{{ route('pp.department.login') }}" class="text-muted text-decoration-none">Change mobile</a>
</div>
@endsection
