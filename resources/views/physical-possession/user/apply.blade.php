...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-2">
    <div class="citizen-card">
        ...
        <div class="p-3">
            ...
            <a href="{{ route('pp.user.download-form') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-emerald-200 bg-emerald-50 text-[11px] font-bold text-emerald-700 no-underline hover:bg-emerald-100 w-full justify-center">
                <span class="material-symbols-outlined text-[16px]">download</span>
                Download Possession Certificate Form
            </a>
            <p class="text-[10px] text-slate-500 mt-2 mb-0">Print, sign, scan and upload this form while applying.</p>
        </div>
    </div>
    ...
    <div class="citizen-card">
        ...
        <div class="p-3">
            <form method="POST" action="{{ route('pp.user.apply.submit') }}" enctype="multipart/form-data" id="ppApplyForm">
                ...
                <div class="mb-2.5">
                    <label class="block text-[11px] font-bold text-slate-700 mb-1">{{ $loop->iteration }}. {{ $label }} <span class="text-red-500">*</span></label>
...
<script>
    ppInitUploadZone('zone_filled_form', 'input_filled_form', 'preview_filled_form');
    ppInitUploadZone('zone_registration_certificate', 'input_registration_certificate', 'preview_registration_certificate');
    ppInitUploadZone('zone_provisional_possession_letter', 'input_provisional_possession_letter', 'preview_provisional_possession_letter');
</script>
@endpush
