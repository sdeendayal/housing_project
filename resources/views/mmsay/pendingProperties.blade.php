                        ...
                        <tbody class="divide-y divide-slate-100 text-xs">
                            @forelse ($properties as $row)
                                @php
                                    ...
                                    $applicationNumber =
                                        data_get($row, 'application_number') ?? data_get($row, 'ApplicationNo');
                                    $applicantName =
                                        data_get($row, 'applicant_name') ?? data_get($row, 'PrivatePurchaserName');
                                    ...
                                    $districtName = data_get($row, 'district_name') ?? data_get($row, 'DistrictName');
                                    $cityName = data_get($row, 'city_name') ?? data_get($row, 'CityName');
                                    $sectorName = data_get($row, 'sector_name') ?? data_get($row, 'SectorName');
                                    $flatCost =
                                        (float) (data_get($row, 'flat_cost') ?? (data_get($row, 'FlatCost') ?? 0));
                                    $totalPaid =
                                        (float) (data_get($row, 'total_paid') ??
                                            (data_get($row, 'total_received') ?? 0));
                                    $pendingAmount =
                                        (float) (data_get($row, 'pending_amount') ?? max($flatCost - $totalPaid, 0));
                                @endphp

                                <tr class="transition hover:bg-slate-50/70">
                                    <td class="px-4 py-3">
                                        <p class="font-semibold text-slate-800">Asset #{{ $assetId ?: '-' }}</p>
                                        <p class="mt-0.5 text-[10px] text-slate-400">App: {{ $applicationNumber ?: '-' }}
                                        </p>
                                    </td>
                                    <td class="px-4 py-3">
