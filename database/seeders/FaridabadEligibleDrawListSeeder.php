...
class FaridabadEligibleDrawListSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $this->command->info("Clearing existing Faridabad records (dist_id = 4) from ews_eligible_draw_list_5 table...");
        DB::table('ews_eligible_draw_list_5')->where('dist_id', 4)->delete();

        $this->command->info("Seeding Faridabad data into ews_eligible_draw_list_5 table (starting from row 2)...");
        ...
        $this->command->info("Successfully seeded {$count} Faridabad records into the ews_eligible_draw_list_5 table in {$duration} seconds.");
    }
}
