...
class RohtakHouseOwnershipRejectSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $this->command->info("Clearing existing Rohtak house ownership records (dist_id = 20) from ews_house_ownership_reject_4 table...");
        DB::table('ews_house_ownership_reject_4')->where('dist_id', 20)->delete();

        $this->command->info("Seeding Rohtak data into ews_house_ownership_reject_4 table (starting from row 2)...");
        ...
        $this->command->info("Successfully seeded {$count} Rohtak records into the ews_house_ownership_reject_4 table in {$duration} seconds.");
    }
}
