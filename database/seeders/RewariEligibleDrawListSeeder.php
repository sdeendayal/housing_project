...
class RewariEligibleDrawListSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $this->command->info("Clearing existing Rewari records (dist_id = 19) from ews_eligible_draw_list_5 table...");
        DB::table('ews_eligible_draw_list_5')->where('dist_id', 19)->delete();

        $this->command->info("Seeding Rewari data into ews_eligible_draw_list_5 table (starting from row 2)...");
        ...
        $this->command->info("Successfully seeded {$count} Rewari records into the ews_eligible_draw_list_5 table in {$duration} seconds.");
