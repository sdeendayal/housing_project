...
class FaridabadHouseOwnershipRejectSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $this->command->info("Clearing existing Faridabad house ownership records (dist_id = 4) from ews_house_ownership_reject_4 table...");
        DB::table('ews_house_ownership_reject_4')->where('dist_id', 4)->delete();

        $this->command->info("Seeding Faridabad data into ews_house_ownership_reject_4 table (starting from row 2)...");
        ...
        $this->command->info("Successfully seeded {$count} Faridabad records into the ews_house_ownership_reject_4 table in {$duration} seconds.");
    }
}
