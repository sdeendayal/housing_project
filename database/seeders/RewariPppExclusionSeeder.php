...
class RewariPppExclusionSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $this->command->info("Clearing existing Rewari PPP Exclusion records (dist_id = 19) from ews_reject_ppp_exclusion_2 table...");
        DB::table('ews_reject_ppp_exclusion_2')->where('dist_id', 19)->delete();

        $this->command->info("Seeding Rewari data into ews_reject_ppp_exclusion_2 table (starting from row 2)...");
        ...
        $this->command->info("Successfully seeded {$count} Rewari records into the ews_reject_ppp_exclusion_2 table in {$duration} seconds.");
    }
}
