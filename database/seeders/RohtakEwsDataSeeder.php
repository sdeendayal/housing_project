...
class RohtakEwsDataSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        // Select the sheet by name 'Rohtak_MC_Complted_09.01.26'
        ...
        $this->command->info("Clearing existing Rohtak records (dist_id = 20) from all_ews_data_1 table...");
        DB::table('all_ews_data_1')->where('dist_id', 20)->delete();

        $this->command->info("Seeding data into all_ews_data_1 table for ROHTAK...");
        ...
        $this->command->info("Successfully seeded {$count} Rohtak records into the all_ews_data_1 table.");

        $this->command->info("Populating member_id and ppt_member_id columns from ppt_members table for Rohtak...");
        try {
            $affectedMemberIds = DB::update("
                ...
                WHERE all_ews_data_1.dist_id = 20
            ");
            $affectedPptMemberIds = DB::update("
                ...
                WHERE all_ews_data_1.dist_id = 20
            ");
            $this->command->info("Successfully populated {$affectedMemberIds} member_id records and {$affectedPptMemberIds} ppt_member_id records for Rohtak.");
        } catch (\Exception $e) {
