...
class RohtakAllEwsDataSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $this->command->info("Clearing existing Rohtak EWS records (dist_id = 20) from all_ews_data_1 table...");
        DB::table('all_ews_data_1')->where('dist_id', 20)->delete();

        $this->command->info("Seeding Rohtak data into all_ews_data_1 table (starting from row 2)...");
        ...
        $this->command->info("Successfully seeded {$count} Rohtak records into the all_ews_data_1 table in {$duration} seconds.");

        $this->command->info("Populating member_id and ppt_member_id columns for Rohtak...");
        try {
