...
class GurugramPropertyRejectSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $this->command->info("Clearing existing Gurugram property reject records (dist_id = 6) from ews_reject_property_in_india_3 table...");
        DB::table('ews_reject_property_in_india_3')->where('dist_id', 6)->delete();

        $this->command->info("Seeding Gurugram data into ews_reject_property_in_india_3 table (starting from row 2)...");
        ...
        $this->command->info("Successfully seeded {$count} Gurugram records into the ews_reject_property_in_india_3 table in {$duration} seconds.");
