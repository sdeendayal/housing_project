...
class RohtakPropertyRejectSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $this->command->info("Clearing existing Rohtak records (dist_id = 20) from ews_reject_property_in_india_3 table...");
        DB::table('ews_reject_property_in_india_3')->where('dist_id', 20)->delete();

        $this->command->info("Seeding Rohtak data into ews_reject_property_in_india_3 table (starting from row 2)...");
        ...
        $this->command->info("Successfully seeded {$count} Rohtak records into the ews_reject_property_in_india_3 table in {$duration} seconds.");
