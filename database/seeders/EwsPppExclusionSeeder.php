...
class EwsPppExclusionSeeder extends Seeder
{
    ...
    {
        $filePath = database_path('seeders/data/ews_exclusion_data.xlsx');
        ...
        $this->command->info("Truncating existing ews_ppp_exclusion table...");
        DB::table('ews_ppp_exclusion')->truncate();

        $this->command->info("Seeding data into ews_ppp_exclusion table (starting from row 3)...");
        ...
        for ($row = 3; $row <= $highestRow; $row++) {
            ...
            if (count($batch) >= $batchSize) {
                DB::table('ews_ppp_exclusion')->insert($batch);
                $count += count($batch);
                $batch = [];
            }
        }
        ...
        $this->command->info("Successfully seeded {$count} records into the ews_ppp_exclusion table.");
    }
}
