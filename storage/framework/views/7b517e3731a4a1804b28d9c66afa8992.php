...
.pp-dash-ring {
    ...
    height: 44px;
    border-radius: 50%;
    background: conic-gradient(#059669 <?php echo e($approvalRate); ?>%, #e2e8f0 0);
    display: flex;
    align-items: center;
    justify-content: center;
    flex-shrink: 0;
...
<div class="pp-dash-welcome">
    <div>
        <h2><i class="bi bi-geo-alt-fill me-1"></i><?php echo e($officer->district_name); ?> District</h2>
        <p><?php echo e($officer->name); ?> &middot; Site Engineer</p>
    </div>
    <span class="pp-dash-date"><i class="bi bi-calendar3 me-1"></i><?php echo e(now()->format('d M Y')); ?></span>
</div>

<div class="pp-dash-stats">
    <a href="<?php echo e(route('pp.officer.possession-applications')); ?>" class="pp-dash-stat">
        <div class="pp-dash-stat-icon indigo"><i class="bi bi-collection"></i></div>
        <div class="min-w-0">
            <div class="pp-dash-stat-label">Total Applications</div>
            <div class="pp-dash-stat-value pp-counter" data-target="<?php echo e($stats['total']); ?>">0</div>
        </div>
    </a>
    <a href="<?php echo e(route('pp.officer.eligibility-list')); ?>" class="pp-dash-stat">
        <div class="pp-dash-stat-icon blue"><i class="bi bi-person-check"></i></div>
        <div class="min-w-0">
            <div class="pp-dash-stat-label">Awaiting Schedule</div>
            <div class="pp-dash-stat-value pp-counter" data-target="<?php echo e($stats['awaiting_schedule']); ?>">0</div>
        </div>
    </a>
    <a href="<?php echo e(route('pp.officer.possession-applications', ['status' => 'Visit Scheduled'])); ?>" class="pp-dash-stat">
        <div class="pp-dash-stat-icon purple"><i class="bi bi-calendar-event"></i></div>
        <div class="min-w-0">
            <div class="pp-dash-stat-label">Visits Scheduled</div>
            <div class="pp-dash-stat-value pp-counter" data-target="<?php echo e($stats['scheduled']); ?>">0</div>
        </div>
    </a>
    <a href="<?php echo e(route('pp.officer.possession-applications', ['status' => 'Physical Possession Submitted'])); ?>" class="pp-dash-stat">
        <div class="pp-dash-stat-icon orange"><i class="bi bi-hourglass-split"></i></div>
        <div class="min-w-0">
            <div class="pp-dash-stat-label">Pending Verify</div>
            <div class="pp-dash-stat-value pp-counter" data-target="<?php echo e($stats['submitted']); ?>">0</div>
        </div>
    </a>
    <a href="<?php echo e(route('pp.officer.possession-applications', ['status' => 'Site Verified'])); ?>" class="pp-dash-stat">
        ...
        <div class="min-w-0">
            ...
            <div class="pp-dash-stat-value pp-counter" data-target="<?php echo e($stats['site_verified']); ?>">0</div>
        </div>
    </a>
    <a href="<?php echo e(route('pp.officer.possession-applications', ['status' => 'Verified'])); ?>" class="pp-dash-stat">
        <div class="pp-dash-stat-icon green"><i class="bi bi-check-circle"></i></div>
        <div class="min-w-0">
            <div class="pp-dash-stat-label">Verified</div>
            <div class="pp-dash-stat-value pp-counter" data-target="<?php echo e($stats['verified']); ?>">0</div>
        </div>
    </a>
</div>

<div class="pp-dash-actions">
    <?php if($stats['submitted'] > 0): ?>
        <a href="<?php echo e(route('pp.officer.possession-applications', ['status' => 'Physical Possession Submitted'])); ?>" class="pp-dash-action warn">
            <i class="bi bi-exclamation-circle"></i> Review <?php echo e($stats['submitted']); ?> Submission<?php echo e($stats['submitted'] !== 1 ? 's' : ''); ?>
...
<div class="card premium-card border-0 shadow-sm mt-3">
    <div class="card-body p-3">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-3">
            ...
            <form action="<?php echo e(route('pp.officer.dashboard')); ?>" method="GET" class="d-flex gap-2 align-items-center mb-0">
                <div class="input-group input-group-sm" style="max-width: 250px;">
                    <span class="input-group-text bg-white border-end-0 text-muted"><i class="bi bi-search"></i></span>
                    <input type="text" name="search" class="form-control border-start-0 ps-0" style="font-size: 0.72rem;" placeholder="Search name, mobile..." value="<?php echo e($search ?? ''); ?>">
                </div>
                <button type="submit" class="btn btn-primary btn-sm px-3 rounded-pill fw-bold" style="font-size: 0.7rem;">Search</button>
                <?php if($search): ?>
                    <a href="<?php echo e(route('pp.officer.dashboard')); ?>" class="btn btn-outline-secondary btn-sm px-2 rounded-pill" style="font-size: 0.7rem;">Reset</a>
                <?php endif; ?>
            </form>
        </div>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $purchasers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <small class="text-muted text-uppercase tracking-wider font-monospace fs-9">PPP ID: <?php echo e($p->PPPId ?? '—'); ?></small>
                            </td>
                            <td>
                            ...
                            <td>
                                <?php if($p->physical_possession_status): ?>
                                    ...
                                    ?>
                                    <span class="badge <?php echo e($badgeClass); ?> px-2 py-1 rounded-3" style="font-size: 0.65rem;">
...
<?php echo $__env->make('physical-possession.layouts.officer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\xampp\htdocs\housing-project\resources\views/physical-possession/officer/dashboard.blade.php ENDPATH**/ ?>